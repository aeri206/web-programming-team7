...
for corp_code in corp_code_list[6099:9500]: # [:N]으로 조절하기. # 9500까지 돌림.
    ...
    f = open("/Users/hyun/PycharmProjects/dart/xmlfile/%s.xml" % file_name, "w")
    f.write(response.text)
    f.close()

    tree = ET.parse("/Users/hyun/PycharmProjects/dart/xmlfile/%s.xml" % file_name) # 이스케이프 코드가 있는 경우 에러가 나옴. ex)로리첸벌스에이/에스
    root = tree.getroot()
    print(root)
